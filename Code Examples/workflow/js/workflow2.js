...
class dataTable {
  ...
  async getRecentChanges() {
    ...
    try { 
      const response = await fetch('recent-changes.json', { method: 'GET', cache: 'no-cache' });
      const data = await response.json();
      this.showRecentChanges(data);
    }
    catch(error) {
      console.log(error);
    }
  ...
  async getOpenIssues() {
    ...
    try {
      const response = await fetch('issues-data.json', { method: 'GET', cache: 'no-cache' });
      const data = await response.json();
      this.showOpenIssues(data);
    }
    catch(error) {
      console.log(error);
    }
  ...
  showOpenIssues(data) {
    ...
    for (const issue of data.issues) {
      str += '<tr data-priority="' + issue.priority.toLowerCase() + '">';
      str += '<th scope="row">' + issue.id + '</th>';
      str += '<td>' + issue.type + '</td>';
      str += '<td><a href="' + issue.project + '-' + issue.id + '.html">' + issue.desc + '</a></td>';
      str += '<td>' + issue.priority + '</td>';
      str += '<td>' + issue.assigned + '</td>';
      str += '</tr>';
    }

    // append the open issues
    document.querySelector('#projectsTbl tbody').insertAdjacentHTML('beforeend', str);
...
// request open issues, update class instance
openItems.getOpenIssues();
